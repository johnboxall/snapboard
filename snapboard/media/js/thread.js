...
function preview(form_id) {
    var urlq = SNAPBOARD_URLS["preview"],
    ...
    YAHOO.util.Connect.asyncRequest('POST', urlq, callback, text);
}

function revision(orig_id, show_id) {
    var urlq = SNAPBOARD_URLS.rpc_postrev + '?orig=' + orig_id + '&show=' + show_id,
    ...
    var handleSuccess = function(o) {
        if(o.responseText !== undefined) {
            ...
            links_html = [];
            if(res['prev_id'] !== '') {
                links_html.push('<a href="#post' + orig_id + '" onClick="revision(\'');
                links_html.push(orig_id + '\',\'' + res['prev_id']);
                links_html.push('\');">&#171; ' + gettext('previous') + '</a>');
            }
            links_html.push(' <b style="color: #c00;">' + gettext('This message has been revised') + '</b> ');
            if(res['rev_id'] !== '') {
                links_html.push('<a href="#post' + orig_id + '" onClick="revision(\'');
                links_html.push(orig_id + '\',\'' + res['rev_id']);
                links_html.push('\');">' + gettext('next') + ' &#187;</a>');
            }
            div_links.innerHTML = Array.join(links_html, '');
        }
    };
    ...
    YAHOO.util.Connect.asyncRequest('GET', urlq, callback, null);
}

function toggle_variable(action, pk, msgdivid) {
    var postData = 'id=' + pk,
        div = document.getElementById(action + pk);
    ...
    YAHOO.util.Connect.asyncRequest('POST', SNAPBOARD_URLS[action], callback, postData);
}


// thread level functions
function set_csticky(id) { toggle_variable('csticky', id, 'thread_rpc_feedback'); }
function set_gsticky(id) { toggle_variable('gsticky',id, 'thread_rpc_feedback'); }
function set_watch(id) { toggle_variable('watch', id, 'thread_rpc_feedback'); }
function set_close(id) { toggle_variable('close', id, 'thread_rpc_feedback'); }

// post level function
function set_censor(id) { toggle_variable('censor', id, ('post_rpc_feedback' + id)); }
function set_abuse(id) { toggle_variable('abuse', id, ('post_rpc_feedback' + id));}

// get the source text of a message to quote it
function get_post_to_quote(pk, cb) {
    var postData = 'id=' + pk;
    var callback = {
        success: function (o) {
            if(o.responseText) {
    ...
    YAHOO.util.Connect.asyncRequest('POST', SNAPBOARD_URLS["quote"], callback, postData);
}
